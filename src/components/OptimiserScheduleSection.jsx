...
import { IconCalendarSidebar, IconPlus, IconReplenishment, IconReorder, IconRebalancing, IconChevronDown, IconList, IconCalendarNote, IconTruck, IconTrendUp, IconLightbulb, IconEdit, IconClose, IconChevronDownSelect } from './icons'
...
export function OptimiserScheduleSection({ onAddJob }) {
        ...
        <div className="bg-[#f3f4f6] border border-[#ebf3ff] rounded-[14px] p-6 flex flex-col gap-5" data-name="Calendar container" data-node-id="174:2767">
          <div className="flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-[16px] font-medium text-[#0a0a0a] leading-tight">Schedule & Jobs</p>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2 ml-auto shrink-0">
              <div className="flex items-center gap-2 relative" data-name="Review status multiselect" data-node-id="12771:5757">
                ...
                {reviewStatusDropdownOpen && (
                  <>
                    ...
                    <div className="absolute left-0 top-full mt-1 z-50 w-full min-w-[200px] bg-white border border-[#e9eaeb] rounded-[4px] p-2 shadow-[0px_8px_25px_0px_rgba(0,0,0,0.12)]" data-name="Dropdown list" data-node-id="12771:5850">
              ...
              <div className="flex items-center gap-2 relative">
                ...
                >
                  <IconCalendarSidebar className="text-[#22272f] size-4 shrink-0" aria-hidden />
                      ...
                      <div className="flex flex-col gap-0">
                        <div className="grid grid-cols-7">
                          {['M', 'T', 'W', 'T', 'F', 'S', 'S'].map((wd) => (
                            <div key={wd} className="size-12 flex items-center justify-center text-[14px] font-medium text-[#4b535c]">{wd}</div>
                          ))}
                        </div>
        ...
        <div className="flex flex-col gap-6">
          ...
          {activeViewOption === 'month' && (
            <div className="border border-[#e5e7eb] rounded-[10px] overflow-visible relative">
              <div className="grid grid-cols-7 bg-[#f3f4f6] border-b border-[#e5e7eb]">
                {weekDays.map((day) => (
                  <div key={day} className="py-3 text-center text-[14px] font-medium text-[#364153] border-r border-[#e5e7eb] last:border-r-0">{day}</div>
                ))}
              </div>
              <div className="grid grid-cols-7">
                {monthGrid.map((row, ri) =>
                  row.map((date, ci) => {
                    ...
                    const byReview = selectedReviewStatuses.length === 0 ? rawCellEntries : rawCellEntries.filter((e) => selectedReviewStatuses.includes(entryReviewStatus[e.id] || 'upcoming'))
                    const cellEntries = activeTypeFilter === 'all' ? byReview : byReview.filter((e) => e.type === activeTypeFilter)
                    const cellDate = getCellDate(ri, ci)
                          ...
                          return (
                            <div key={entry.id} className="relative group mt-1 w-fit">
                              <div
                                ...
                                onClick={() => {
                                  if (pinnedHoverEntryId === entry.id && pinnedHoverCellKey === cellKey) { setPinnedHoverEntryId(null); setPinnedHoverCellKey(null) }
                                  else { setPinnedHoverEntryId(entry.id); setPinnedHoverCellKey(cellKey) }
                                }}
                              ...
                              >
                                <div className="p-4 flex flex-col gap-3">
                                  <div className="flex items-start justify-between gap-2">
                                    <div className="flex items-center gap-2">
                                      <span className="flex items-center justify-center w-8 h-8 rounded-[8px] bg-[#ebf3ff] text-[#0267ff]"><Icon className="size-4" /></span>
                                      <div>
                                        <p className="text-[14px] font-medium text-[#0a0a0a]">{entry.title}</p>
                                        <p className="text-[12px] text-[#4b535c]">{monthNames[entry.startDate.getMonth()]} {entry.startDate.getDate()} – {entry.endDate.getDate()}, {entry.endDate.getFullYear()}</p>
                                      </div>
                                    </div>
                                    <button type="button" onClick={() => openDrawerForEdit(entry)} className="shrink-0 h-8 px-3 rounded-[4px] text-[13px] font-medium text-[#0267ff] hover:bg-[#ebf3ff]">Edit schedule</button>
                                  </div>
                                  ...
                                  <button type="button" onClick={() => openDrawerForEdit(entry)} className="w-full h-10 px-4 rounded-[4px] bg-[#0267ff] text-white text-[16px] font-medium flex items-center justify-center gap-2 shrink-0">
                                    <IconEdit />
                                    Edit Job
                                  </button>
                                </div>
          ...
          {activeViewOption === 'week' && (
            <div className="border border-[#e5e7eb] rounded-[10px] overflow-hidden">
              ...
              <div className="grid grid-cols-7">
                {weekRow.map((d, i) => (
                  <div key={i} className="min-h-[80px] p-2 border-r border-[#e5e7eb] bg-white text-[14px] text-[#0a0a0a] last:border-r-0">{d.getDate()}</div>
                ))}
              </div>
            </div>
          )}
          {activeViewOption === 'list' && (
            <div className="border border-[#e5e7eb] rounded-[10px] overflow-hidden">
              <div className="bg-[#f3f4f6] border-b border-[#e5e7eb] py-3 px-4 text-[14px] font-medium text-[#364153]">{monthNames[viewDate.getMonth()]} {viewDate.getFullYear()} – list</div>
        ...
        <>
          ...
          <div className="fixed right-0 top-0 bottom-0 w-[800px] bg-white shadow-xl z-50 flex flex-col" role="dialog" aria-modal aria-labelledby="add-schedule-title" data-name={editingScheduleEntry ? 'Edit schedule' : 'Add Schedule'} data-node-id="214:2622">
            ...
            <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6">
              <section className="flex flex-col gap-2">
                ...
                <div className="relative">
                  <button type="button" onClick={() => setModuleDropdownOpen((o) => !o)} className={`w-full h-10 flex items-center justify-between gap-2 px-3 rounded-[4px] border bg-white text-[14px] text-left ${moduleDropdownOpen ? 'border-[#0267ff]' : 'border-[#e9eaeb]'}`} data-name="Input multiple select" data-node-id="12770:4659">
                    <span className={drawerForm.modules.length === 0 ? 'text-[#4b535c]' : 'text-[#0a0a0a]'}>
                      {drawerForm.modules.length === 0 ? 'Select' : drawerForm.modules.map((id) => MODULE_OPTIONS.find((o) => o.id === id)?.label).filter(Boolean).join(', ')}
                    </span>
                  ...
                  {moduleDropdownOpen && (
                    <>
                      ...
                      <div className="absolute left-0 top-full mt-1 z-[70] w-full min-w-[200px] bg-white border border-[#e9eaeb] rounded-[4px] p-2 shadow-[0px_8px_25px_0px_rgba(0,0,0,0.12)]" data-name="Dropdown list" data-node-id="12771:5850">
                        {MODULE_OPTIONS.map((opt) => {
                          const selected = drawerForm.modules.includes(opt.id)
                          return (
                            <button key={opt.id} type="button" onClick={() => toggleModule(opt.id)} className="w-full flex gap-2 items-center p-3 rounded-[3px] text-left hover:bg-[#f8f8f8] focus:bg-[#f8f8f8]" data-name="Dropdown item" data-node-id="12771:5851">
                              <span className="flex items-center justify-center shrink-0 size-6">
                                <span className={`flex items-center justify-center rounded-[4px] size-5 border-2 ${selected ? 'bg-[#0267ff] border-[#0267ff]' : 'bg-white border-[#e5e7eb]'}`}>
                                  {selected && (<svg width="12" height="12" viewBox="0 0 12 12" fill="none" className="text-white"><path d="M2 6l3 3 5-6" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg>)}
                                </span>
                              </span>
                              <span className="flex-1 text-[12px] font-medium text-[#0a0a0a] leading-normal">{opt.label}</span>
                            </button>
            ...
            <footer className="flex items-center justify-end gap-3 shrink-0 p-6 border-t border-[#e9eaeb]">
              <button type="button" onClick={closeDrawer} className="h-10 px-4 rounded-[4px] text-[16px] font-medium text-[#0a0a0a] hover:bg-[#f3f4f6]">Cancel</button>
              <button type="button" className="h-10 px-4 rounded-[4px] bg-[#0267ff] text-white text-[16px] font-medium">{editingScheduleEntry ? 'Save changes' : 'Add Schedule'}</button>
            </footer>
          </div>
        </>
      )}
